...
-- Tabel functions: setiap fungsi yang terdeteksi
...
-- Tabel call_edges: call graph (siapa panggil siapa)
...
-- Tabel taint_flows: hasil taint analysis (source → sink)
...
CREATE INDEX idx_taint_flows_source ON taint_flows(source_finding_id);
CREATE INDEX idx_taint_flows_sink ON taint_flows(sink_finding_id);
CREATE INDEX idx_taint_flows_is_sanitized ON taint_flows(is_sanitized);